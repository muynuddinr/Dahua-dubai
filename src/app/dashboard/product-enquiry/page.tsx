...
export default function ProductEnquiryPage() {
  ...
  const fetchEnquiries = async () => {
    try {
      ...
      // Filter only PRODUCT enquiries
      const productEnquiries = response.data.filter((enq: Enquiry) => enq.productName && enq.productName.trim() !== '');
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (enq) =>
          ...
          (enq.productName && enq.productName.toLowerCase().includes(query))
      );
    }
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-bold text-slate-900">Product Enquiries</h1>
        <p className="text-slate-600 mt-2">Manage customer product enquiries and requests</p>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by name, email, phone, company, product..."
                ...
                <div className="flex-1 space-y-3">
                  ...
                  {enquiry.productName && (
                    <div className="flex items-center gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                      {enquiry.productId && typeof enquiry.productId === 'object' && enquiry.productId.images && enquiry.productId.images.length > 0 ? (
                        <img
                          src={enquiry.productId.images[0].url}
                      ...
                      <div>
                        <p className="text-xs text-slate-600">Product Enquiry</p>
                        <p className="text-sm font-semibold text-emerald-700">{enquiry.productName}</p>
                      </div>
                    </div>
                  )}
            ...
            {selectedEnquiry.productName && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-6 mb-6">
                ...
                <div className="flex items-center gap-4">
                  {selectedEnquiry.productId && typeof selectedEnquiry.productId === 'object' && selectedEnquiry.productId.images && selectedEnquiry.productId.images.length > 0 ? (
                    <img
                      src={selectedEnquiry.productId.images[0].url}
                    ...
                    <div className="w-24 h-24 bg-emerald-100 rounded-lg flex items-center justify-center">
                      <FaBox className="w-12 h-12 text-emerald-600" />
                    </div>
                  )}
                  <div>
                    <p className="text-slate-900 font-bold text-lg">{selectedEnquiry.productName}</p>
                    {selectedEnquiry.productSlug && (
                      <p className="text-slate-600 text-sm font-mono">{selectedEnquiry.productSlug}</p>
                    )}
                  </div>
                </div>
              </div>
